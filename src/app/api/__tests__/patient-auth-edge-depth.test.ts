...
describe('POST /api/patient/auth/verify edge depth', () => {
  ...
  it('blocks disallowed origins before rate limiting', async () => {
  ...
  it('rejects oversized request bodies before rate limiting', async () => {
  ...
  it('normalizes token email before Airtable lookup', async () => {
    verifyMagicLinkTokenMock.mockResolvedValueOnce({ email: ' PATIENT@EXAMPLE.COM ' });
    const { POST } = await import('@/app/api/patient/auth/verify/route');
    const response = await POST(verifyRequest() as never);

    expect(response.status).toBe(200);
    expect(fetchFirstMock).toHaveBeenCalledWith(
      expect.anything(),
      1,
      expect.objectContaining({
        filterByFormula: expect.stringContaining("{Email} = 'patient@example.com'"),
      }),
      true,
    );
  });
