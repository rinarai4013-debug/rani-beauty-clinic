...
describe('dashboard growth + ops routes', () => {
  ...
  it('GET /api/dashboard/briefing returns 500 when upstream aggregators fail', async () => {
    ...
    expect(body.error).toBe('Failed to build briefing');
  });
  ...
  it('GET /api/dashboard/reactivation returns 500 when upstream intelligence fetch fails', async () => {
    ...
    expect(response.status).toBe(500);
    expect(body.error).toBe('Failed to load reactivation intelligence');
  });
});
