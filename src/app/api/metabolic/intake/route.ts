...
export async function POST(req: NextRequest) {
  return withSentry('metabolic/intake', async () => {
    ...
    const programBundle = buildUnifiedIntakeDecisionBundle(intake);
    const recommendation = programBundle.primary.recommendation;
    const packageOptions = programBundle.primary.packages;
    const dosagePlan = programBundle.primary.dosagePlan;
    const trajectory = programBundle.primary.trajectory;
    ...
    const intakeSummary = [
      ...
      `Alternative Tracks: ${programBundle.alternatives.map((program) => program.track).join(', ')}`,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        trajectory,
        programBundle,
        nextSteps: programBundle.primary.checkout,
      },
    });
  });
}
