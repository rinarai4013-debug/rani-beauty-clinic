...
import { buildAchievementsPayload } from '@/lib/dashboard/gamification-summary';
...
export async function GET() {
  ...
  try {
    const payload = await buildAchievementsPayload();
    cache.set(cacheKey, payload, TTL.STANDARD);
    return NextResponse.json(payload);
  } catch (error) {
    console.error('[dashboard/gamification/achievements]', error);
