  ...
  return withSentry('plan/[id]', async () => {
    try {
        ...
        return NextResponse.json(
          {
            error: 'PLAN_EXPIRED',
            message:
              'This treatment plan has expired. Please contact us at (425) 539-4440 for a refreshed plan.',
      ...
      const plan = {
        ...
        intakeSummary:
          intelligence?.['Intake Summary (AI)'] || intake['Intake Summary (AI)'] || null,
        programPlan: intelligence?.['Program Plan (AI)'] || intake['Program Plan (AI)'] || null,
        costBreakdown:
          intelligence?.['Cost Breakdown (AI)'] || intake['Cost Breakdown (AI)'] || null,
        timeline: intelligence?.['Timeline (AI)'] || intake['Timeline (AI)'] || null,
        suggestedNextStep:
          intelligence?.['Suggested Next Step (AI)'] || intake['Suggested Next Step (AI)'] || null,
        treatmentValue:
          intelligence?.['Treatment Value (AI)'] || intake['Treatment Value (AI)'] || null,

        skinHealthScore: intelligence?.['Skin Health Score'] || null,
        projectedScore: intelligence?.['Projected Score'] || null,

        intelligenceReady:
          !!intelligence || !!(intake['Intake Summary (AI)'] || intake['Program Plan (AI)']),

        // Expiry info for client display
        expiresAt: createdDate
          ? new Date(
              new Date(createdDate).getTime() + PLAN_EXPIRY_DAYS * 24 * 60 * 60 * 1000,
            ).toISOString()
          : null,
      };
