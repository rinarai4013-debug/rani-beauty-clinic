...
export async function GET() {
  return withSentry('mastermind/aura-import', async () => {
    try {
      ...
      logEvent('api', 'error', '[Aura Import API] GET error', {
...
export async function POST(request: NextRequest) {
  return withSentry('mastermind/aura-import', async () => {
    try {
      ...
      logEvent('api', 'info', '[Aura Import API] Importing scan', {
        patientName,
        scanDate: scanDate || 'latest',
      });
      ...
      logEvent('ai', 'info', '[Aura Import API] Import complete', {
        auraScoreOverall: scanResult?.auraScore?.overall,
        auraScoreGrade: scanResult?.auraScore?.grade,
      });
      ...
      logEvent('api', 'error', '[Aura Import API] POST error', {
        error: error instanceof Error ? error.message : String(error),
      });
      return apiError(
