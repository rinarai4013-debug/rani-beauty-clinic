...
// ── Sanitized Types (patient-facing only) ──

interface PatientAuraScore {
  overall: number;
  grade: string;
  label: string;
  skinAge: number;
...
interface PatientTreatment {
  ...
  downtime: string;
  riskLevel: string;
  aiReasoning: string; // patient-facing reasoning only
  synergiesWith: string[];
}

interface PatientSequenceItem {
  phase: number;
  phaseName: string;
  duration: string;
...
interface PatientDeviceAnalysis {
  categories: {
    label: string;
...
interface PatientPredictiveMetrics {
  withoutIntervention: {
