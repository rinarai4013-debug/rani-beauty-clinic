...
export default function ConsultCopilotPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-heading text-rani-navy">AI Consult Co-pilot</h1>
        <p className="text-sm font-body text-rani-muted mt-1">Real-time treatment recommendations, objection handling, and closing strategies</p>
      </div>
      ...
      {briefing && (
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-body font-semibold uppercase tracking-wider text-rani-gold">
              Client Intelligence Briefing
            </h3>
            <span className={`px-2.5 py-1 rounded-full text-[10px] font-semibold uppercase border ${SEGMENT_COLORS[briefing.segment]}`}>
              {briefing.segment.replace('_', ' ')}
            </span>
          </div>
          <p className="text-sm font-body text-white/80 mb-4">{briefing.summary}</p>
        ...
        {plan && (
          <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-rani-border p-5">
            ...
            <motion.div
              ...
              className="p-4 rounded-lg bg-rani-gold/5 border-2 border-rani-gold/20 mb-3"
            >
              ...
              <p className="text-sm font-body font-semibold text-rani-navy">{plan.primary.service}</p>
            ...
            {plan.alternatives.map((alt, i) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-body font-semibold text-rani-navy">{alt.service}</span>
                ...
                {plan.packages.map((pkg, i) => (
                  <div key={i} className="p-3 rounded-lg bg-green-50/50 border border-green-200/50 mb-2">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[10px] font-body font-semibold text-green-600">Save ${pkg.savings}</span>
                    </div>
                ...
                <div className="space-y-2">
                  {plan.timeline.map((step, i) => (
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs font-body font-semibold text-rani-navy">{step.treatment}</p>
              ...
              return (
                <div key={timing}>
                  ...
                  {timingPoints.map((point, i) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-body font-semibold text-rani-navy">{point.topic}</span>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded border font-medium ${PRIORITY_STYLES[point.priority]}`}>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-rani-border p-5">
            ...
            <div className="space-y-3">
              {objections.map((obj, i) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    <Shield className="w-3.5 h-3.5 text-rani-gold" />
                    <span className="text-xs font-body font-semibold text-red-600">&ldquo;{obj.objection}&rdquo;</span>
                  </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-rani-border p-5">
            ...
            <div className="space-y-3">
              {crossSells.map((cs, i) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-body font-semibold text-rani-navy">{cs.service}</span>
                    <span className="text-xs font-body font-semibold text-green-600">+${cs.addOnPrice}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-[10px] font-body text-rani-muted">Conversion Likelihood:</span>
                    <ProgressBar current={cs.conversionLikelihood} target={100} showPercentage={false} height={4} colorMode="gold" />
                    <span className="text-[10px] font-body font-semibold text-rani-navy">{cs.conversionLikelihood}%</span>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {closing && (
          <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-rani-border p-5">
            ...
            <div className="p-4 rounded-lg bg-rani-gold/5 border border-rani-gold/20">
              <div className="flex items-center gap-2 mb-2">
