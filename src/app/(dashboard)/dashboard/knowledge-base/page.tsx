...
export default function KnowledgeBasePage() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-heading text-rani-navy">RAG Knowledge Base</h1>
        <p className="text-sm font-body text-rani-muted mt-1">Treatment protocols, aftercare guides, FAQs, and clinic knowledge for AI-powered responses</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-rani-border p-5 flex flex-col justify-center">
          <p className="text-[10px] font-body font-semibold text-rani-muted uppercase tracking-wider mb-2">Index Health</p>
          <span className={`inline-block w-fit px-3 py-1.5 rounded-full text-sm font-body font-semibold capitalize ${HEALTH_COLORS[indexHealth]}`}>
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between mb-1">
                      <span className="text-xs font-body font-medium text-rani-navy capitalize">{category.replace(/_/g, ' ')}</span>
                      <span className="text-xs font-body font-semibold text-rani-navy">{count}</span>
                    </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-rani-border p-5">
          ...
          <div className="space-y-3">
            {Object.entries(byService).map(([service, count], i) => (
              ...
              >
                <span className="text-xs font-body font-semibold text-rani-navy">{service}</span>
      ...
      {gaps.length > 0 && (
        <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-rani-border p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            {highGaps > 0 && (
              <span className="text-[10px] px-2 py-0.5 rounded bg-red-50 text-red-700 font-semibold">
            ...
            {gaps.map((gap, i) => (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs font-body font-semibold">{gap.area}</span>
                </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <p className="text-sm font-body font-semibold text-white">Pinecone</p>
          </div>
          <div>
            <p className="text-[10px] font-body text-white/40 uppercase">Index</p>
            <p className="text-sm font-body font-semibold text-white">rani-knowledge-base</p>
          </div>
          <div>
            <p className="text-[10px] font-body text-white/40 uppercase">Embedding Model</p>
            <p className="text-sm font-body font-semibold text-white">text-embedding-3-small (1536d)</p>
          </div>
        </div>
