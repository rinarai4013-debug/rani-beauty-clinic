...
export function generateStaticParams() {
  return serviceGeoEntries.map((entry) => ({
    city: entry.citySlug,
...
export function generateMetadata({ params }: PageProps): Metadata {
  const entry = getServiceGeoEntry(params.city, params.service);
  if (!entry) {
...
export default function ServiceCityPage({ params }: PageProps) {
  const entry = getServiceGeoEntry(params.city, params.service);
  const city = pnwCities.find((c) => c.slug === params.city);
  const template = serviceTemplates.find((t) => t.slug === params.service);

  if (!entry || !city || !template) {
    notFound();
  }

  const otherServicesInCity = getServiceGeoByCity(params.city).filter(
    (s) => s.serviceSlug !== params.service
  );

  const sameSvcOtherCities = serviceGeoEntries
