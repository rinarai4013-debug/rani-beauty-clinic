...
export function generateStaticParams() {
  return pnwCities.map((city) => ({
    city: city.slug,
  }));
}

function getCityBySlug(slug: string): PNWCity | undefined {
  return pnwCities.find((c) => c.slug === slug);
}
...
export default function NearCityPage({ params }: PageProps) {
  ...
  const cityServices = nearServiceList.filter((s) => s.citySlug === city.slug);
  const nearbyCities = pnwCities
