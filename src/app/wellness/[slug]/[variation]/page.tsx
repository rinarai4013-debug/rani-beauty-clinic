...
const wellnessVariations = serviceVariations.filter(
...
export function generateMetadata({ params }: PageProps): Metadata {
  ...
  return {
    ...
    alternates: {
      canonical: `${clinicInfo.website}/wellness/${params.slug}/${params.variation}`,
    },
    openGraph: {
      title: variation.metaTitle,
      description: variation.metaDescription,
      type: "website",
      url: `${clinicInfo.website}/wellness/${params.slug}/${params.variation}`,
      images: [{ url: "/opengraph-image", width: 1200, height: 630, alt: `${variation.metaTitle} - Rani Beauty Clinic` }],
    },
  };
}

export default function WellnessVariationPage({ params }: PageProps) {
  const variation = wellnessVariations.find(
    (v) => v.parentSlug === params.slug && v.slug === params.variation
  );
  ...
  const siblings = wellnessVariations.filter(
    (v) => v.parentSlug === params.slug && v.slug !== params.variation
  );
  ...
  const breadcrumbs = [
    ...
    { name: "Wellness", url: `${clinicInfo.website}/wellness` },
    {
      name: variation.parentTitle,
      url: `${clinicInfo.website}/wellness/${variation.parentSlug}`,
    },
    {
      name: variation.title,
      url: `${clinicInfo.website}/wellness/${variation.parentSlug}/${variation.slug}`,
    },
  ];

  return (
    <>
      <ServicePageTracker serviceName={variation.title} serviceCategory="wellness" />
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          >
            <ol className="flex flex-wrap items-center gap-2">
              ...
              <li>
                <Link
                  href={`/wellness/${variation.parentSlug}`}
                  className="hover:text-rani-navy transition-colors"
                >
          ...
          <div className="mx-auto max-w-7xl px-6">
            ...
            <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {siblings.slice(0, 6).map((sib) => (
                <Link
                  key={sib.slug}
                  href={`/wellness/${sib.parentSlug}/${sib.slug}`}
      ...
      <CTABanner
        label={variation.parentTitle.toUpperCase()}
        title={`Ready to Start ${variation.title}?`}
