...
const EXPENSE_KEYWORDS: Record<ExpenseCategory, string[]> = {
  ...
  rent: ['rent', 'lease', 'mortgage', 'property'],
...
function generatePnL(
  ...
  // Period comparison
  const totalPeriodExpenses = cos + totalOpex;
  ...
  if (input.previousPeriod) {
    ...
    periodComparison = {
      ...
      expenseChange:
        prev.expenses > 0
          ? Math.round(((totalPeriodExpenses - prev.expenses) / prev.expenses) * 100)
...
function projectCashFlow(
  ...
  const projectionGrowth = {
    revenueGrowthRate: clampGrowth(input.projectionGrowthRates?.revenueGrowthRate, 0.03),
    expenseGrowthRate: clampGrowth(input.projectionGrowthRates?.expenseGrowthRate, 0.01, 0, 0.5),
  };

  const start = new Date(`${input.period.start}T00:00:00Z`);
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const projectedRev = Math.round(monthlyRevenue * Math.pow(1 + projectionGrowth.revenueGrowthRate, i));
    const projectedExp = Math.round(monthlyExpenses * Math.pow(1 + projectionGrowth.expenseGrowthRate, i));
    const projectedNet = projectedRev - projectedExp;
    const projectedBalance = balance + projectedNet;

    // Runway: if losing money (burn > 0), how many months until zero
    const monthlyBurn = projectedExp - projectedRev;
    const runway = monthlyBurn > 0 && balance > 0 ? Math.max(0, Math.floor(balance / monthlyBurn)) : monthlyBurn > 0 ? 0 : 99;
...
function calculateKPIs(input: FinanceInput, pnl: ProfitLossStatement): FinancialKPIs {
  const periodStart = new Date(`${input.period.start}T00:00:00Z`);
  const periodEnd = new Date(`${input.period.end}T00:00:00Z`);
  const days = Math.max(
    1,
...
function clampGrowth(
  ...
  const bounded = Number.isFinite(raw) ? Math.max(min, Math.min(max, raw)) : fallback;
  return bounded;
}
