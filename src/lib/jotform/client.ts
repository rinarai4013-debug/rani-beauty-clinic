...
const getJotformApiKey = () => env.JOTFORM_API_KEY;
...
async function jotformFetch<T>(endpoint: string): Promise<T> {
  const jotformApiKey = getJotformApiKey();
  if (!jotformApiKey) {
    throw new Error('JOTFORM_API_KEY not configured');
  }

  const url = `${BASE_URL}${endpoint}${endpoint.includes('?') ? '&' : '?'}apiKey=${jotformApiKey}`;
...
export function isConfigured(): boolean {
  return Boolean(getJotformApiKey());
}
