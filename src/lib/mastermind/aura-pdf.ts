...
async function loadPdfJsModule(): Promise<PdfJsModule> {
  if (!pdfJsPromise) {
    pdfJsPromise = (async () => {
      const modulePath = path.join(process.cwd(), 'public', 'vendor', 'pdfjs', 'pdf.min.mjs');
      const workerPath = path.join(process.cwd(), 'public', 'vendor', 'pdfjs', 'pdf.worker.min.mjs');
...
export async function extractAuraPdfInsights(
  ...
  return {
    textSummary: text.slice(0, maxChars),
    absoluteScores,
    peerScores,
    peerSkinScore,
