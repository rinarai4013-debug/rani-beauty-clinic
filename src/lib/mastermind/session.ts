...
export function sessionReducer(
  ...
  switch (action.type) {
    ...
    case 'SET_PROTOCOL_PACKET':
      return {
        ...state,
        updatedAt: now,
        protocolPacket: {
          packetUrl: action.packetUrl,
          generatedAt: action.generatedAt,
          generatorActor: action.generatorActor,
          packetVersion: action.packetVersion,
        } satisfies ProtocolPacketMeta,
        activityLog: appendLog(
          state.activityLog,
          'protocol_packet_generated',
          `Protocol packet v${action.packetVersion} generated by ${action.generatorActor}`,
          action.generatorActor,
        ),
      };
...
function hydrateSession(parsed: Record<string, unknown>): MastermindSession {
  return {
    ...
    protocolPacket: parsed.protocolPacket as ProtocolPacketMeta | undefined,
  };
}
